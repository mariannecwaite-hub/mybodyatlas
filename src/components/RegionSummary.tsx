    ...
    <AnimatePresence>
      ...
      >
          ...
          <div className="space-y-1 mb-4">
            {regionEvents.slice(0, 3).map((event) => (
              <button
                ...
                className="w-full flex items-center gap-2.5 p-2.5 rounded-xl text-left hover:bg-secondary/50 transition-all duration-200 group"
              >
                <span className="text-sm">{typeIcons[event.type]}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-foreground/85 truncate">{event.title}</p>
                  <p className="text-[11px] text-muted-foreground/55">
                    {new Date(event.date).toLocaleDateString("en-US", { month: "short", year: "numeric" })}
                    {event.ongoing && <span className="ml-1 text-sage-foreground/70">· ongoing</span>}
                  </p>
                </div>
                <span className="opacity-0 group-hover:opacity-50 text-muted-foreground text-xs">→</span>
              </button>
            ))}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-border text-[12px] text-muted-foreground/60 hover:text-muted-foreground hover:border-primary/20 transition-all duration-200"
        >
