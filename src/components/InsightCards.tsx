...
const InsightCards = () => {
  ...
  function generateInsights() {
    ...
    const symptomEvents = state.events.filter((e) => e.type === "symptom");
    const treatmentEvents = state.events.filter((e) => e.type === "treatment");
    const injuryEvents = state.events.filter((e) => e.type === "injury");
    const lifeEvents = state.events.filter((e) => e.type === "life-event");

    // --- Stress-symptom overlap ---
    // Check if any stress event's date range (±3 months) overlaps with a symptom in the same body region
    ...
    if (overlappingRegions.length > 0) {
      const regionNames = overlappingRegions.slice(0, 2).map((r) => r.replace("-", " ")).join(" and ");
      cards.push({
    ...
    const relatedPairs: Record<string, string[]> = {
      "left-foot": ["left-knee", "left-hip", "lower-back"],
      "right-foot": ["right-knee", "right-hip", "lower-back"],
      "left-knee": ["left-hip", "lower-back"],
      "right-knee": ["right-hip", "lower-back"],
    };
    ...
    // --- Postpartum / life-event + new symptom ---
    const postpartumEvent = lifeEvents.find((e) => e.title.toLowerCase().includes("born") || e.description.toLowerCase().includes("delivery") || e.description.toLowerCase().includes("birth"));
    if (postpartumEvent) {
      ...
      if (postpartumSymptoms.length > 0) {
        cards.push({
          ...
          body: `New symptoms appeared in the months after a significant life event. Your body was adapting to a lot — physically and emotionally. That takes time, and you're allowed to be patient with it.`,
          tone: "warm",
        });
      }
    }
    ...
    if (ongoingCount > 0) {
      ...
      if (ongoingTreatments > 0) {
        cards.push({
          ...
          body: `You have ${ongoingCount} ongoing ${ongoingCount === 1 ? "thread" : "threads"}, including ${ongoingTreatments} active ${ongoingTreatments === 1 ? "treatment" : "treatments"}. Staying with the process is itself a form of care.`,
          tone: "sage",
        });
      } else {
        cards.push({
    ...
    // --- Timeline breadth ---
    const dates = state.events.map((e) => new Date(e.date).getFullYear());
